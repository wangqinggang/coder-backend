...
//		String apiTag = entity.getApiTag(); // Controller 接口说明 API（tags） 注解用
		...
		// 实体的属性列表
		List<EntityField> columns = entity.getEntityFeilds();
...
//						.add(AnnotationSpec.builder(GeneratedValue.class).addMember("strategy", "auto").build());
				...
				if (column.getDateType().equals("String")) {
				...
				FieldSpec android = FieldSpec.builder(this.getFieldType(column.getDateType()), column.getFieldName())
...
//			    @JsonFormat(pattern = Constants.PATTERN_DATE, timezone = "GMT+8")
				...
				if (column.getDateType().equals("Date")) {
				...
				FieldSpec android = FieldSpec.builder(getFieldType(column.getDateType()), column.getFieldName())
						.addModifiers(Modifier.PRIVATE).addAnnotations(fieldAnnotations).build();
		...
		JavaFile javaFile = JavaFile.builder("com.ideaworks.club.domain", classTypeSpec).build();
...
//		    @JsonFormat(pattern = Constants.PATTERN_DATE, timezone = "GMT+8")
		...
		JavaFile javaFile = JavaFile.builder("com.ideaworks.club.dto", classTypeSpec).build();
...
//		String apiTag = entity.getApiTag(); // Controller 接口说明 API（tags） 注解用
		...
		// 实体的属性列表
		List<EntityField> columns = entityDTO.getEntityFeilds();

		ClassName idClassName = null;
		for (EntityField column : columns) {
			if (column.getIsKey() == true) {
				idClassName = ClassName.bestGuess(column.getDateType());
			}
		}
		if (idClassName == null) {
		...
		// 接口继承 jpa 接口
//		ClassName modelClass = ClassName.get(("com.ideaworks.club.domain").trim(), entityName);
...
//				.get(JpaRepository.class.getTypeName().concat("<" + modelClass + ", " + primaryKey + ">"));
		...
		ClassName PageingClassName = ClassName.get("org.springframework.data.jpa.repository",
		...
		TypeSpec repositorySpec = TypeSpec.interfaceBuilder(entityName + "Repository").addModifiers(Modifier.PUBLIC)
				.addSuperinterface(ParameterizedTypeName.get(jpaClassName,
						ClassName.get("com.ideaworks.club.domain", entityName), ClassName.get("java.lang", "String")))
				.addSuperinterface(ParameterizedTypeName.get(PageingClassName,
						ClassName.get("com.ideaworks.club.domain", entityName), ClassName.get("java.lang", "String")))
				.build();
		JavaFile javaFile = JavaFile.builder("com.ideaworks.club.repository", repositorySpec).build();
...
//		String apiTag = entity.getApiTag(); // Controller 接口说明 API（tags） 注解用
		...
		// 实体类的类名
		ClassName modelClass = ClassName.get(("com.ideaworks.club.domain").trim(), entityName);
		ClassName DTOClass = ClassName.get(("com.ideaworks.club.dto").trim(), entityName);
		// 实体的属性列表
		List<EntityField> columns = entityDTO.getEntityFeilds();
		...
		getMethodParameters.add(ParameterSpec.builder(SCGServiceImpl.getFieldType(columnEntityField.getDateType()),
		...
		MethodSpec GetByIdMethodSpec = MethodSpec
				...
				.addModifiers(modifiers).returns(modelClass).addParameters(getMethodParameters).build();
		...
		// 要更新的新实体
		updateMethodParameters.add(ParameterSpec.builder(modelClass, entityName.toLowerCase()).build());

		// 要更新的实体名
		updateMethodParameters.add(ParameterSpec.builder(SCGServiceImpl.getFieldType(columnEntityField.getDateType()),
				columnEntityField.getFieldName().toLowerCase()).build());
		...
		MethodSpec UpdateByIdMethodSpec = MethodSpec
				.methodBuilder(
						"update" + entityName + "By" + columnEntityField.getFieldName().substring(0, 1).toUpperCase()
								+ columnEntityField.getFieldName().substring(1))
				.addModifiers(modifiers).returns(modelClass).addParameters(updateMethodParameters).build();
		...
		deleteMethodParameters.add(ParameterSpec.builder(SCGServiceImpl.getFieldType(columnEntityField.getDateType()),
				columnEntityField.getFieldName().toLowerCase()).build());
		...
		isExitMethodParameters.add(ParameterSpec.builder(SCGServiceImpl.getFieldType(columnEntityField.getDateType()),
				columnEntityField.getFieldName().toLowerCase()).build());
		...
		ClassName condition = ClassName.get("com.ideaworks.club.dto", entityName + "DTO");
		...
		JavaFile javaFile = JavaFile.builder("com.ideaworks.club.service", serviceTypeSpec).build();
...
//		String apiTag = entity.getApiTag(); // Controller 接口说明 API（tags） 注解用
		...
		// 实体类的类名
		ClassName DTOClass = ClassName.get(("com.ideaworks.club.form").trim(), entityName + "DTO");

		ClassName serviceInterface = ClassName.get(("com.ideaworks.club.service").trim(),
				entityName.toLowerCase() + "Service");

		ClassName repositoryClass = ClassName.get(("com.ideaworks.club.service").trim(),
...
//		modifiers.add(Modifier.ABSTRACT);
		...
		MethodSpec saveMethodSpec = MethodSpec.methodBuilder("save" + entityName).addModifiers(modifiers)
				.returns(DTOClass)
				.addStatement("final $T " + entityName.toLowerCase() + " = new $T;\n" + "mapToEntity("
						+ entityName.toLowerCase() + "DTO," + entityName.toLowerCase() + ");\n"
						+ ("return " + (entityName.toLowerCase()) + "Repository" + ".save(" + entityName.toLowerCase()
								+ ").mapToDTO(" + entityName.toLowerCase() + "," + "new " + entityName.toLowerCase()
								+ "DTO())").trim(),
...
//		TypeName modelClasses = ParameterizedTypeName.get(list, modelClass);
		...
		MethodSpec GetAllMethodSpec = MethodSpec.methodBuilder("getAll" + entityName + "s").addModifiers(modifiers)
				.addStatement("return " + entityName.toLowerCase() + "Repository" + ".findAll().stream.map("
						+ entityName.toLowerCase() + "->mapToDTO( " + entityName.toLowerCase() + "," + "new "
						+ entityName.toLowerCase() + "DTO()).collect(Collectors.toList())")
		...
		MethodSpec GetByIdMethodSpec = MethodSpec
				...
				.addStatement("return " + entityName.toLowerCase() + "Repository" + ".findById("
						+ columnEntityField.getFieldName().toLowerCase() + ").mapToDTO(" + entityName.toLowerCase()
						+ "," + "new " + entityName.toLowerCase() + "DTO())"
		...
		MethodSpec UpdateByIdMethodSpec = MethodSpec
				.methodBuilder(
						"update" + entityName + "By" + columnEntityField.getFieldName().substring(0, 1).toUpperCase()
								+ columnEntityField.getFieldName().substring(1))
				.addModifiers(modifiers)
				.addStatement("final " + entityName + " " + entityName.toLowerCase() + " = " + entityName.toLowerCase()
						+ "Repository" + ".findById(" + columnEntityField.getFieldName().toLowerCase() + ").mapToDTO("
						+ entityName.toLowerCase() + "," + "new " + entityName.toLowerCase() + "DTO())"
						+ ".orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND);\n" + "mapToEntity("
						+ entityName + "DTO ," + entityName.toLowerCase() + ");\n" + "return " + entityName
		...
		MethodSpec isExitMethodSpec = MethodSpec.methodBuilder("is" + entityName + "Exist").addModifiers(modifiers)
				.addStatement(entityName.toLowerCase() + "Repository" + ".existsById("
						+ columnEntityField.getFieldName().toLowerCase() + ")")
				.returns(Boolean.class).addParameters(isExitMethodParameters).build();
		...
		ClassName listPage = ClassName.get("org.springframework.data.domain", "Page");
		ClassName condition = ClassName.get("com.ideaworks.club.form", entityName + "DTO");
		ClassName pageable = ClassName.get("org.springframework.data.domain", "Pageable");
		...
		getPageMethodParameters.add(ParameterSpec.builder(condition, (entityName + "DTO").toLowerCase())
		...
		for (int i = 0; i < columns.size(); i++) {
			EntityField column = columns.get(i);
			String typeString = column.getDateType();
			if (typeString.equals("String")) {
				codeBlockString += "if(!StringUtils.isEmpty(" + column.getFieldName() + ")){\n"
						+ "\tPredicate predicate = cb.equal(root.get('" + column.getFieldName() + "'),"
						+ column.getFieldName() + "); \n" + "\tlist.add(predicate);\n}\n";
			}else {
				codeBlockString += "if(column.getFieldName()!=null){\n"
						+ "\t$Predicate predicate = cb.equal(root.get('" + column.getFieldName() + "'),"
						+ column.getFieldName() + "); \n" + "\tlist.add(predicate);\n}\n";
			}
		}
		...
		MethodSpec specificationMethodSpec = MethodSpec.methodBuilder("createSpecification").addModifiers(modifiers)
				.addStatement(
						"return (root,query,criteriaBuilder) - > {\n" + "List<$T> predicates = new ArrayList<$T>();\n"
								+ "if(!StringUtils.isEmpty(" + columnEntityField.getFieldName() + ")){ \n"
								+ "\t$T predicate = cb.equal(root.get('" + columnEntityField.getFieldName() + "'),"
								+ columnEntityField.getFieldName() + "); \n" + "\tlist.add(predicate);\n" + "}\n" 
								+ codeBlockString
								+ "return query.where(predicates.toArray(new Predicate[predicates.size()])).getRestriction();"
								+"}",
						predicateName, predicateName, predicateName)
				.addParameter(ParameterSpec.builder(condition, (entityName + "DTO").toLowerCase())
						.addModifiers(Modifier.FINAL).build())
...
//		}
		...
		JavaFile javaFile = JavaFile.builder("com.ideaworks.club.serviceImpl", serviceImplTypeSpec).build();
...
//		javaFile.writeTo(System.out);
	...
	public String generateController(EntityDTO entityDTO) {
		...
		getMethodParameters
				.add(ParameterSpec
						.builder(SCGServiceImpl.getFieldType(columnEntityField.getDateType()),
								columnEntityField.getFieldName().toLowerCase())
						.addAnnotation(
		...
		FileBuilderFactory.build(
				("com.ideaworks.club.controller").trim(),
				classTypeSpec);
